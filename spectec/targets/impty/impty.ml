...
let quickcheck_command =
  ...
  fun () ->
    ...
    Quickcheck.Driver.check
      ~target:(module T)
      ~generalize ~max_steps ~num_tests
...
let fuzz_command =
  ...
  and max_size =
    flag "--max-size"
      ...
      ~doc:"N maximum size of generated programs (default 50)"
  and out_dir =
    flag "--out-dir"
  ...
  fun () ->
    ...
    Quickcheck.Driver.check
      ~target:(module T)
      ~generalize:false ~max_steps ~num_tests ~max_size
      ~on_counterexample:(save_counterexample ~out_dir)
      ~manual_gens:Manual_gen.manual_gens lang qc
    |> Result.map_error (fun e ->
           Error.QuickcheckError (Quickcheck.Driver.error_to_string e))
