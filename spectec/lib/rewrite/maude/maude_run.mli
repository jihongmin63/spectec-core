...
(** The batch separator {!run_batch}/[run_batch_direct] delimit each start's
    output with: a quoted [String] literal for [run_batch]'s [Native] path (its
    module always imports [STRING]);
    {!To_mfe.module_of_system]'s
    ...
    imports [BOOL]). Exposed so {!To_mfe}
    can declare a matching constant. *)
...
(** [run_direct ?maude_bin ?timeout ?defined_heads ~mode ~module_text ~start ()]
    ...
    through its real signature -- no [metaReduce] reflection wrapper is written.
    This is the {!Ctrs_term.scalar_theory}.[Structural] counterpart of {!run}
    (which is [Native]-only, via {!To_maude.meta_term_of_value}): a structural
    value has no built-in wrapper for Maude to reflect specially, so it cannot
    go through the META-TERM grammar. [Search] is not supported (only
    [Reduce]/[Rewrite]). *)
val run_direct :
  ?maude_bin:string ->
  ?timeout:int ->
