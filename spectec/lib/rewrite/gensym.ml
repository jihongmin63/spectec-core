(* Make the stateful gensym builtins ([$fresh_typeId] / p4-old's [$fresh_tid])
   ...
   Each effectful call occurrence counts as one issuance -- {!Prem_env} keeps
   such calls out of the equivalence classes so {!Simplify} never duplicates
   one instance into many. *)
...
let root_syms : string list =
  List.map
    (fun name -> T.func_sym Common.Source.(name $ no_region))
    Prem_env.gensym_ids
