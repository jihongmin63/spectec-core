(* The IL -> rewriting-system translation, in two pipelines that share the SAME
   ...
   Both pipelines wrap the core translation with the feature passes:
     - {!translated_spec} FIRST -- {!Defunctionalize.defunctionalize} so every
       call is first-order, then {!Monomorphize.monomorphize} so no type
       parameter survives -- before simplification/translation see it.
...
(* The spec the translation actually sees. Both passes RENAME and duplicate
   definitions ([$f] with a [def] parameter becomes one [$f_g] per instantiated
   argument; [$f<K>] becomes one [$f_<T>] per instantiated type), so a consumer
   that recovers a symbol's declared signature -- {!Maude_sorts} through
   {!Spec_index}, and the slice roots {!To_ctrs.def_symbols} lists -- must read
   the spec from HERE. Reading the elaborated spec instead finds no declaration
   under the emitted names, which silently widens every argument sort to [Val]
   and names slice roots that no rule defines.

   One-slot memo by physical equality, like the passes it chains, and a hit on
   its own RESULT too: a caller that translates the spec and then feeds it back
   into a pipeline gets the same value, so the per-spec indices
   ({!Spec_index.of_spec}) stay shared instead of being rebuilt against a
...
let translated_spec (spec : Lang.Il.spec) : Lang.Il.spec =
  ...
  | _ ->
      let tr =
        Monomorphize.monomorphize (Defunctionalize.defunctionalize spec)
      in
      memo := Some (spec, tr);
      tr
