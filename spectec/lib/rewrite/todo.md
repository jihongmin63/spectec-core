...
```
  ...
  → $write_value_from_bits' n_var=0 경계                                       [CRC MAYBE 5 + term MAYBE/TIMEOUT 4의 뿌리이자 유일한 진짜 비합류: 2.1.2-value-aux.spectec:147-153의 두 절이 n_var=0에서 동시에 발화. 원 스펙의 절 순서가 지던 disambiguation을 번역이 잃음 → n_var≠0 가드(또는 owise) 복원 필요]
