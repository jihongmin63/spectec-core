# P4 structural CTRS — 검증 결과 (표)

> **표만 둔다.** 범례: **CRC** = Church-Rosser(합류성) — `YES` / `YES*`(=`--crc-normalize` upgrade-only로 닫힘) / `MAYBE` / `TIMEOUT` / `-`(미도달). **ChC(Coherence)는 2026-07-24부로 측정 중단·열 삭제**(사유·이력은 notes).
> **term** = 구조 보존 unravel → AProVE 직접. **CRC초** = 심볼당 **직렬 fresh** 측정
> 벽시계 초(`>1800` = 예산 내 미완).
>
> **term예산 / term초 — 둘 다 "답이 나온 예산 단계" 하나만 말한다.** term은 예산 사다리
> (5·20·80·…·cap)를 올라가며 AProVE를 반복 호출하고 처음 답이 나온 단계에서 멈춘다.
> **term예산** = 그 단계의 예산으로, `YES`에 대해서는 증명 난이도의 상한이다.
> **term초** = 그 단계 한 번의 벽시계로, 아래에서 실패한 단계들은 빠져 있다(`-` = AProVE를
> 돌리지 않음). 그래서 `term초`가 `term예산`보다 짧을 수 있고(AProVE가 마감 전에 답한 경우),
> 조금 길 수도 있다(마감까지 탐색 + JVM 기동·종료 ~1초). 순위·비교에는 **term예산**,
> 개별 증명 비용에는 **term초**를 봐라.
>
> 재현 커맨드·방법론·측정 이력·비-YES 해석·병렬
> 안전성 소견은 모두 **[verification-notes.md](verification-notes.md)**.
>
> **측정 기준**: CRC는 `bff805ec` (2026-07-23 전수 재측정) — 통합 커밋 `160fef97`이
> 분석 표면을 byte-identical로 보존하므로 유효하다. term은 `a1fd043b` (2026-07-24
> 예산 사다리 도입 후 전수 재측정, cap 1800, `$write_bits_from_value`는 AProVE 증명
> export 스택오버플로 회피에 JVM `-Xss512m`). **종전 term초는 예산 설정값이었다** —
> AProVE가 자기 마감 시각에 판정을 발표해 값이 최대 320배 부풀려져 있었다(경위는 notes).

## 1. ≤500 종합 (153심볼)

**요약**: CRC YES 146 (정규화 5 포함) / TIMEOUT 7 · term YES **153 / 153** · 비합류·비종료 후보 0. 잔여 CRC TIMEOUT 7(전부 비트벡터 산술)은 도구 한계(자세히는 notes).
**term 예산 분포**: 5 → 151심볼 / 20 → 2심볼. 답한 단계의 벽시계 합 279.8초.


| symbol | rules | CRC | term | CRC초 | term초 | term예산 |
|---|---|---|---|---|---|---|
| `$annotationList_of_parameterIR` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$ctk_of_typedExpressionIR` | 1 | YES | YES | 0.1 | 0.5 | 5 |
| `$empty_map` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_set` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_tableContext` | 1 | YES | YES | 0.1 | 0.5 | 5 |
| `$id_of_parameterIR` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$invalidate_header` | 1 | YES | YES | 0.1 | 0.5 | 5 |
| `$parameterListIR_of_actionDef` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$parameterListIR_of_actionTypeDefIR` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$parameterListIR_of_constructorTypeDefIR` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$parameterListIR_of_controlApplyMethodDef` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$parameterListIR_of_controlApplyMethodTypeIR` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$parameterListIR_of_definedFunctionDef` | 1 | YES | YES | 0.1 | 0.5 | 5 |
| `$parameterListIR_of_definedFunctionTypeDefIR` | 1 | YES | YES | 0.1 | 0.5 | 5 |
| `$parameterListIR_of_externFunctionDef` | 1 | YES | YES | 0.1 | 0.5 | 5 |
| `$parameterListIR_of_externFunctionTypeDefIR` | 1 | YES | YES | 0.1 | 0.5 | 5 |
| `$parameterListIR_of_parserApplyMethodDef` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$parameterListIR_of_parserApplyMethodTypeIR` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$parameterListIR_of_tableApplyMethodDef` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$parameterListIR_of_tableApplyMethodTypeDefIR` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$tableEntryPriorityOptIR_of_tableEntryIR` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$type_of_typedExpressionIR` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$type_of_typedLvalueIR` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$parameterListIR_of_externMethodDef` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$set_priority_of_tableEntryIR` | 1 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_callableDefEnv` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_callableTypeDefEnv` | 2 | YES | YES | 0.1 | 0.5 | 5 |
| `$empty_constructorDefEnv` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_constructorTypeDefEnv` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_frame` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_stateEnv` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_store` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_theta` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_typeDefEnv` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_typeFrame` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_constOpt` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$ite` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_objectInitializerOptIR` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$is_some` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$opt_as_seq` | 2 | YES | YES | 0.1 | 0.5 | 5 |
| `$parameterListIR_of_externMethodTypeDefIR` | 2 | YES | YES | 0.1 | 0.5 | 5 |
| `$type_of_externMethodPrototypeIR` | 2 | YES | YES | 0.1 | 0.4 | 5 |
| `$callable_builtinMethod` | 3 | YES | YES | 0.1 | 0.4 | 5 |
| `$constructorTypeDef_of_externConstructorPrototypeIR` | 3 | YES | YES | 0.1 | 0.5 | 5 |
| `$constructor_of_externConstructorPrototypeIR` | 3 | YES | YES | 0.1 | 0.5 | 5 |
| `$empty_constraint` | 3 | YES | YES | 0.1 | 0.4 | 5 |
| `$instantiable_extern` | 3 | YES | YES | 0.1 | 0.5 | 5 |
| `$is_lpm_key_prime` | 3 | YES | YES | 0.1 | 0.5 | 5 |
| `$un_lnot` | 3 | YES | YES | 0.1 | 0.4 | 5 |
| `$join_tableEntryState` | 3 | YES | YES | 0.1 | 0.4 | 5 |
| `$filter` | 3 | YES | YES | 0.1 | 0.5 | 5 |
| `$is_concrete_extern_object_prime_prime` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$is_default_parameterIR` | 4 | YES | YES | 0.1 | 0.5 | 5 |
| `$is_lpm_key` | 4 | YES | YES | 0.1 | 0.5 | 5 |
| `$concat_text` | 4 | YES | YES | 0.1 | 0.5 | 5 |
| `$exists` | 4 | YES | YES | 0.1 | 0.5 | 5 |
| `$flatten_blockElementStatementList` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_controlLocalDeclarationList` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_externConstructorOrMethodPrototypeList` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_objectDeclarationList` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_parserLocalDeclarationList` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_parserStatementList` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_selectCaseList` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_switchCaseList` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_tableActionList` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_tableEntryList` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_tableKeyList` | 4 | YES | YES | 0.1 | 0.5 | 5 |
| `$flatten_tablePropertyList` | 4 | YES | YES | 0.1 | 0.5 | 5 |
| `$flatten_typeFieldList` | 4 | YES | YES | 0.1 | 0.5 | 5 |
| `$forall` | 4 | YES | YES | 0.1 | 0.5 | 5 |
| `$join_flow` | 4 | YES | YES | 0.1 | 0.5 | 5 |
| `$flatten_prefixedNameIR` | 4 | YES | YES | 0.1 | 0.4 | 5 |
| `$add_action_tbl` | 5 | YES | YES | 0.2 | 0.5 | 5 |
| `$add_key_tbl` | 5 | YES | YES | 0.1 | 0.4 | 5 |
| `$codom_map` | 5 | YES | YES | 0.1 | 0.5 | 5 |
| `$dom_map` | 5 | YES | YES | 0.1 | 0.5 | 5 |
| `$enter_path_i` | 5 | YES | YES | 0.1 | 0.4 | 5 |
| `$flatten_p4program` | 5 | YES | YES | 0.1 | 0.4 | 5 |
| `$empty_typingContext` | 6 | YES | YES | 0.1 | 0.5 | 5 |
| `$is_tableActionsProperty` | 6 | YES | YES | 0.1 | 0.5 | 5 |
| `$is_tableKeysProperty` | 6 | YES | YES | 0.1 | 0.4 | 5 |
| `$tableCustomName` | 6 | YES | YES | 0.1 | 0.5 | 5 |
| `$enter_i` | 7 | YES | YES | 0.1 | 0.4 | 5 |
| `$enter_t` | 7 | YES | YES | 0.1 | 0.4 | 5 |
| `$exit_i` | 7 | YES | YES | 0.2 | 0.4 | 5 |
| `$exit_t` | 7 | YES | YES | 0.1 | 0.5 | 5 |
| `$requires_priority_prime` | 7 | YES | YES | 0.2 | 0.5 | 5 |
| `$empty_instContext` | 8 | YES | YES | 0.1 | 0.5 | 5 |
| `$requires_priority` | 8 | YES | YES | 0.2 | 0.5 | 5 |
| `$typedLvalueIR_as_typedExpressionIR` | 8 | YES | YES | 0.2 | 0.5 | 5 |
| `$join_ctk` | 9 | YES | YES | 0.1 | 0.4 | 5 |
| `$width_of_integerTypeIR` | 10 | YES | YES | 0.2 | 0.4 | 5 |
| `$inherit_i` | 10 | YES | YES | 0.2 | 0.5 | 5 |
| `$name` | 10 | YES | YES | 0.2 | 0.5 | 5 |
| `$un_plus` | 11 | YES | YES | 0.2 | 0.4 | 5 |
| `$callableId_IR` | 13 | YES | YES | 42.7 | 0.5 | 5 |
| `$objectId_ends_with` | 11 | YES | YES | 0.2 | 0.5 | 5 |
| `$callableId_of_externConstructorPrototypeIR` | 14 | YES | YES | 75.4 | 0.5 | 5 |
| `$prefixedTypeName` | 12 | YES | YES | 0.2 | 0.5 | 5 |
| `$join_text` | 13 | YES | YES | 0.2 | 0.6 | 5 |
| `$resolve_constraint` | 9 | YES | YES | 0.2 | 0.5 | 5 |
| `$callableId_of_externMethodPrototypeIR` | 15 | YES | YES | 132.2 | 0.5 | 5 |
| `$flatten_nameList` | 13 | YES | YES | 0.2 | 0.5 | 5 |
| `$flatten_typeParameterList` | 13 | YES | YES | 0.2 | 0.4 | 5 |
| `$assignop_as_binop` | 13 | YES | YES | 0.2 | 0.4 | 5 |
| `$flatten_typeParameterListOpt` | 15 | YES | YES | 0.2 | 0.4 | 5 |
| `$is_tableDefaultActionProperty` | 16 | YES | YES | 0.3 | 0.5 | 5 |
| `$prefixedNonTypeName` | 19 | YES | YES | 0.3 | 0.5 | 5 |
| `$optional_annotation_of_parameterIR_prime_prime` | 20 | YES | YES | 0.4 | 0.5 | 5 |
| `$lvalue_as_expression` | 22 | YES | YES | 0.5 | 0.5 | 5 |
| `$starts_with` | 50 | YES | YES | 1.5 | 0.5 | 5 |
| `$strip_prefix_rec` | 53 | YES | YES | 1.7 | 0.5 | 5 |
| `$isValid_header` | 80 | YES | YES | 4.8 | 0.6 | 5 |
| `$ends_with` | 88 | YES | YES | 4.9 | 0.6 | 5 |
| `$strip_suffix_rec` | 91 | YES | YES | 5.5 | 0.6 | 5 |
| `$invalidate_headerUnion` | 87 | YES | YES | 6.6 | 0.8 | 5 |
| `$invalidate_value` | 87 | YES | YES | 6.5 | 0.8 | 5 |
| `$write_bits_from_value` | 103 | TIMEOUT | YES | >1800 | 6.0 | 5 |
| `$bin_mod` | 109 | YES | YES | 7.6 | 0.6 | 5 |
| `$bin_div` | 113 | YES | YES | 8.3 | 0.6 | 5 |
| `$un_bnot` | 139 | YES | YES | 14.1 | 5.5 | 5 |
| `$bin_ge` | 183 | YES | YES | 30.1 | 5.6 | 5 |
| `$bin_le` | 183 | YES | YES | 29.9 | 5.6 | 5 |
| `$bin_gt` | 184 | YES | YES | 30.0 | 5.6 | 5 |
| `$bin_lt` | 184 | YES | YES | 29.9 | 5.6 | 5 |
| `$int_of_integerValue` | 184 | YES | YES | 29.6 | 5.6 | 5 |
| `$nat_of_integerValue` | 187 | YES | YES | 31.2 | 5.6 | 5 |
| `$bin_minus` | 193 | YES | YES | 36.1 | 5.6 | 5 |
| `$bin_mul` | 193 | YES | YES | 36.3 | 5.7 | 5 |
| `$bin_plus` | 193 | YES | YES | 36.3 | 5.6 | 5 |
| `$un_minus` | 197 | YES | YES | 37.5 | 5.6 | 5 |
| `$bin_bxor` | 199 | YES | YES | 43.6 | 5.6 | 5 |
| `$bin_concat` | 200 | YES | YES | 425.7 | 5.6 | 5 |
| `$set_priorities_of_tableEntryListIR_prime` | 200 | YES | YES | 43.7 | 5.7 | 5 |
| `$bin_satminus` | 201 | YES | YES | 46.0 | 5.7 | 5 |
| `$bin_satplus` | 201 | YES | YES | 45.8 | 5.9 | 5 |
| `$bin_shl` | 201 | TIMEOUT | YES | >1800 | 5.6 | 5 |
| `$bin_band` | 202 | YES | YES | 142.9 | 5.7 | 5 |
| `$bin_bor` | 202 | YES | YES | 143.3 | 5.7 | 5 |
| `$name_annotationToken` | 209 | YES | YES | 53.3 | 5.9 | 5 |
| `$un_op` | 209 | YES | YES | 47.9 | 5.6 | 5 |
| `$bin_shr` | 215 | TIMEOUT | YES | >1800 | 5.8 | 5 |
| `$set_priorities_of_tableEntryListIR` | 226 | YES | YES | 63.7 | 5.7 | 5 |
| `$name_annotation_opt` | 256 | YES | YES | 99.5 | 6.1 | 5 |
| `$write_value_field_from_bits_prime` | 271 | YES* | YES | 244.1 | 5.7 | 5 |
| `$write_value_fields_from_bits_prime` | 271 | YES* | YES | 243.9 | 5.7 | 5 |
| `$write_value_from_bits_prime` | 271 | YES* | YES | 244.0 | 5.7 | 5 |
| `$write_values_from_bits_prime` | 271 | YES* | YES | 242.8 | 5.7 | 5 |
| `$write_value_from_bits` | 274 | YES* | YES | 253.0 | 5.7 | 5 |
| `$bitacc_range_op` | 283 | TIMEOUT | YES | >1800 | 21.6 | 20 |
| `$bitacc_offset_op` | 285 | TIMEOUT | YES | >1800 | 21.6 | 20 |
| `$bitacc_range_replace_op` | 391 | TIMEOUT | YES | >1800 | 5.6 | 5 |
| `$bitacc_offset_replace_op` | 394 | TIMEOUT | YES | >1800 | 5.7 | 5 |

## 2. >500 슬라이스 (bigsweep · 124/124 측정)

**요약 (term, 124심볼)**: YES 124. 비종료 후보 0. 전부 예산 5, 답한 단계의 벽시계 합 479.6초.
**CRC 열은 옛 bigsweep 값을 그대로 옮긴 것**(이번에 재측정한 축은 term뿐, `-`는 미도달).

| # | symbol | rules | CRC | term | term초 | term예산 |
|---|---|---|---|---|---|---|
| 1 | `$flatten_namedExpressionList` | 748 | YES | YES | 1.4 | 5 |
| 2 | `$flatten_realTypeArgumentList` | 748 | YES | YES | 1.5 | 5 |
| 3 | `$flatten_expressionList` | 749 | TIMEOUT | YES | 1.5 | 5 |
| 4 | `$flatten_typeArgumentList` | 749 | TIMEOUT | YES | 1.4 | 5 |
| 5 | `$expression_as_lvalue` | 766 | MAYBE | YES | 4.0 | 5 |
| 6 | `$flatten_argumentList` | 784 | TIMEOUT | YES | 5.9 | 5 |
| 7 | `$flatten_simpleKeysetExpressionList` | 789 | TIMEOUT | YES | 1.5 | 5 |
| 8 | `$flatten_forUpdateStatementList` | 790 | YES | YES | 2.3 | 5 |
| 9 | `$is_singleton_list_expression` | 812 | YES | YES | 1.7 | 5 |
| 10 | `$add_annotationList` | 867 | YES | YES | 1.8 | 5 |
| 11 | `$flatten_annotationList` | 868 | YES | YES | 2.5 | 5 |
| 12 | `$flatten_parameterList` | 874 | YES | YES | 2.4 | 5 |
| 13 | `$flatten_constructorParameterListOpt` | 876 | YES | YES | 2.5 | 5 |
| 14 | `$is_externConstructorPrototype` | 880 | YES | YES | 1.8 | 5 |
| 15 | `$is_externMethodPrototype` | 883 | YES | YES | 1.9 | 5 |
| 16 | `$callableId_prime` | 884 | YES | YES | 2.7 | 5 |
| 17 | `$callableId` | 885 | YES | YES | 2.5 | 5 |
| 18 | `$constructorId_of_externConstructorPrototype` | 886 | YES | YES | 2.5 | 5 |
| 19 | `$callableId_of_externMethodPrototype` | 887 | YES | YES | 2.5 | 5 |
| 20 | `$constructorId` | 887 | YES | YES | 2.7 | 5 |
| 21 | `$expressionNonBrace_as_expression` | 887 | TIMEOUT | YES | 2.1 | 5 |
| 22 | `$optional_annotation_of_parameterIR_prime` | 893 | YES | YES | 2.0 | 5 |
| 23 | `$optional_annotation_of_parameterIR` | 895 | YES | YES | 2.2 | 5 |
| 24 | `$is_optional_parameterIR` | 896 | YES | YES | 2.2 | 5 |
| 25 | `$flatten_forInitStatementList` | 907 | YES | YES | 2.8 | 5 |
| 26 | `$split_externConstructorOrMethodPrototypeList` | 940 | YES | YES | 2.3 | 5 |
| 27 | `$flatten_parserStateList` | 1029 | YES | YES | 0.6 | 5 |
| 28 | `$name_annotation` | 1125 | - | YES | 6.0 | 5 |
| 29 | `$name_annotation_default` | 1127 | - | YES | 6.1 | 5 |
| 30 | `$cast_header_stack` | 1192 | - | YES | 0.6 | 5 |
| 31 | `$cast_header` | 1194 | - | YES | 0.7 | 5 |
| 32 | `$cast_struct` | 1194 | - | YES | 0.6 | 5 |
| 33 | `$compat_lnot` | 1194 | - | YES | 5.7 | 5 |
| 34 | `$nestable_constructor_package` | 1194 | - | YES | 5.8 | 5 |
| 35 | `$resolve_type_alias` | 1194 | - | YES | 5.6 | 5 |
| 36 | `$callTargetKey_prime` | 1195 | - | YES | 0.6 | 5 |
| 37 | `$compat_bnot` | 1195 | - | YES | 5.6 | 5 |
| 38 | `$compat_divmod` | 1195 | - | YES | 5.6 | 5 |
| 39 | `$compat_logical` | 1195 | - | YES | 5.7 | 5 |
| 40 | `$cast_bool` | 1196 | - | YES | 5.7 | 5 |
| 41 | `$compat_array_index` | 1196 | - | YES | 5.6 | 5 |
| 42 | `$compat_bitslice_offset_index` | 1196 | - | YES | 5.7 | 5 |
| 43 | `$compat_bitslice_offset_width` | 1196 | - | YES | 5.7 | 5 |
| 44 | `$compat_bitslice_range_index` | 1196 | - | YES | 5.6 | 5 |
| 45 | `$compat_uplusminus` | 1196 | - | YES | 5.6 | 5 |
| 46 | `$nestable_constructor_control` | 1196 | - | YES | 5.8 | 5 |
| 47 | `$nestable_constructor_parser` | 1196 | - | YES | 5.7 | 5 |
| 48 | `$nestable_controlApplyMethod` | 1196 | - | YES | 5.6 | 5 |
| 49 | `$nestable_headerStack` | 1196 | - | YES | 5.8 | 5 |
| 50 | `$nestable_headerUnion` | 1196 | - | YES | 5.6 | 5 |
| 51 | `$definable_constructor` | 1197 | - | YES | 5.7 | 5 |
| 52 | `$nestable_constructor_extern` | 1197 | - | YES | 5.6 | 5 |
| 53 | `$nestable_externFunction` | 1197 | - | YES | 5.6 | 5 |
| 54 | `$nestable_externMethod` | 1197 | - | YES | 5.7 | 5 |
| 55 | `$nestable_new_in_enum_serializable` | 1197 | - | YES | 5.6 | 5 |
| 56 | `$nestable_parserApplyMethod` | 1197 | - | YES | 5.6 | 5 |
| 57 | `$compat_switch` | 1198 | - | YES | 5.7 | 5 |
| 58 | `$compat_table_lpm_ternary_range_key` | 1198 | - | YES | 5.6 | 5 |
| 59 | `$nestable_new` | 1198 | - | YES | 5.6 | 5 |
| 60 | `$parameterListIR_of_functionTypeDefIR` | 1198 | - | YES | 0.6 | 5 |
| 61 | `$typedExpressionIR_as_typedLvalueIR` | 1198 | - | YES | 0.6 | 5 |
| 62 | `$compat_concat` | 1199 | - | YES | 5.6 | 5 |
| 63 | `$callTargetKey` | 1200 | - | YES | 0.6 | 5 |
| 64 | `$compat_table_exact_optional_key` | 1202 | - | YES | 5.6 | 5 |
| 65 | `$callableTypeIR_of_callableTypeDefIR` | 1203 | - | YES | 0.6 | 5 |
| 66 | `$compat_shift` | 1203 | - | YES | 5.9 | 5 |
| 67 | `$nestable_enum_serializable` | 1203 | - | YES | 5.7 | 5 |
| 68 | `$typeParameterListIR_of_callableTypeDefIR` | 1203 | - | YES | 0.6 | 5 |
| 69 | `$flatten_keysetExpressionIR` | 1206 | - | YES | 0.6 | 5 |
| 70 | `$is_static_assert_callableTypeIR` | 1206 | - | YES | 0.6 | 5 |
| 71 | `$nestable_tuple_in_set` | 1206 | - | YES | 5.6 | 5 |
| 72 | `$parameterListIR_of_methodTypeDefIR` | 1206 | - | YES | 0.6 | 5 |
| 73 | `$typeId_of_typeDefIR` | 1207 | - | YES | 0.6 | 5 |
| 74 | `$typeParameterListIR_of_typeDefIR` | 1207 | - | YES | 0.6 | 5 |
| 75 | `$nestable_sequence_in_set` | 1208 | - | YES | 5.6 | 5 |
| 76 | `$nestable_struct_in_header` | 1208 | - | YES | 5.6 | 5 |
| 77 | `$nestable_tuple` | 1208 | - | YES | 5.7 | 5 |
| 78 | `$nestable_struct` | 1209 | - | YES | 5.8 | 5 |
| 79 | `$nestable_definedFunction` | 1211 | - | YES | 5.6 | 5 |
| 80 | `$nestable_action` | 1212 | - | YES | 5.8 | 5 |
| 81 | `$nestable_list` | 1212 | - | YES | 5.7 | 5 |
| 82 | `$is_equalable_typeIR` | 1213 | - | YES | 5.7 | 5 |
| 83 | `$typeIR_of_typeDefIR` | 1213 | - | YES | 0.6 | 5 |
| 84 | `$is_assignable_typeIR` | 1214 | - | YES | 5.8 | 5 |
| 85 | `$nestable_typedef` | 1215 | - | YES | 5.7 | 5 |
| 86 | `$init_tableKeys` | 1216 | - | YES | 1.4 | 5 |
| 87 | `$compat_bitslice_base` | 1218 | - | YES | 5.9 | 5 |
| 88 | `$nestable_header` | 1218 | - | YES | 5.8 | 5 |
| 89 | `$is_defaultable_typeIR` | 1219 | - | YES | 5.7 | 5 |
| 90 | `$parameterListIR_of_callableTypeDefIR` | 1221 | - | YES | 0.6 | 5 |
| 91 | `$unroll_typeIR` | 1229 | - | YES | 5.7 | 5 |
| 92 | `$is_table_application` | 1231 | - | YES | 5.7 | 5 |
| 93 | `$nestable_set` | 1235 | - | YES | 5.9 | 5 |
| 94 | `$sizeof_minSizeInBits_prime` | 1259 | - | YES | 5.8 | 5 |
| 95 | `$sizeof_minSizeInBits` | 1260 | - | YES | 5.7 | 5 |
| 96 | `$unroll_aliasType` | 1268 | - | YES | 5.7 | 5 |
| 97 | `$result_concat` | 1269 | - | YES | 5.7 | 5 |
| 98 | `$find_local_return_type_t` | 1270 | - | YES | 0.7 | 5 |
| 99 | `$is_concrete_extern_object_prime` | 1278 | - | YES | 0.7 | 5 |
| 100 | `$sizeof_maxSizeInBits_prime` | 1283 | - | YES | 5.7 | 5 |
| 101 | `$sizeof_maxSizeInBits` | 1284 | - | YES | 5.7 | 5 |
| 102 | `$is_monomorphic_typeDefIR` | 1289 | - | YES | 0.7 | 5 |
| 103 | `$is_polymorphic_typeDefIR` | 1292 | - | YES | 0.6 | 5 |
| 104 | `$resolve_inference_prime` | 1301 | - | YES | 5.7 | 5 |
| 105 | `$parameterListIR_of_functionDef` | 1304 | - | YES | 0.6 | 5 |
| 106 | `$resolve_inference` | 1306 | - | YES | 5.6 | 5 |
| 107 | `$reduce_serenum` | 1310 | - | YES | 5.6 | 5 |
| 108 | `$is_concrete_extern_object` | 1317 | - | YES | 5.7 | 5 |
| 109 | `$update_mode_tbl` | 1337 | - | YES | 5.7 | 5 |
| 110 | `$sizeof_minSizeInBytes` | 1347 | - | YES | 5.7 | 5 |
| 111 | `$sizeof_maxSizeInBytes` | 1350 | - | YES | 5.7 | 5 |
| 112 | `$sizeof` | 1375 | - | YES | 5.8 | 5 |
| 113 | `$init_tableEntries` | 1406 | - | YES | 1.4 | 5 |
| 114 | `$is_valid_bitslice` | 1431 | - | YES | 5.7 | 5 |
| 115 | `$init_table` | 1462 | - | YES | 1.4 | 5 |
| 116 | `$parameterListIR_of_methodDef` | 1551 | - | YES | 0.7 | 5 |
| 117 | `$parameterListIR_of_callableDef` | 1565 | - | YES | 0.6 | 5 |
| 118 | `$parameterListIR_of_constructorDef` | 1570 | - | YES | 0.6 | 5 |
| 119 | `$subexpressions_of_argumentIR` | 1614 | - | YES | 5.7 | 5 |
| 120 | `$subexpressions_of_argumentListIR` | 1614 | - | YES | 5.7 | 5 |
| 121 | `$subexpressions_of_expressionIR` | 1614 | - | YES | 5.6 | 5 |
| 122 | `$subexpressions_of_typedExpressionIR` | 1614 | - | YES | 5.7 | 5 |
| 123 | `$subexpressions_of_typedExpressionListIR` | 1614 | - | YES | 5.6 | 5 |
| 124 | `$name_expression` | 1840 | - | YES | 6.2 | 5 |
