...
비어 자명). 스윕의 NO는 0건이나, 잔여 분류가 **경계-입력 실루프 2종을 증인으로
확보**했다(아래 — `$bitstr_to_int` w=0, `strip_*_rec` 빈 패턴). 번역 결함이
아니라 스펙 자체의 경계값 결함이고, 그중 **w=0은 끝-대-끝 도달까지 확정됐다**:
`const bit<0> A = 0; const bit<0> B = A + A;` 두 줄이 이 스펙의 타입체커를
실제로 발산시킨다(WITNESSES.md ⑸ — `bit<0>` 선언 자체는 수용되고, `$bin_plus`의
W 절이 decode를 부른다). 빈-패턴 쪽은 호출부가 리터럴 접두사를 넘겨 도달 불가로
추정된다(미확정).
...
- 플립 175건의 규칙 수 중앙값은 1,221, 최대는 `Cast_impl` 3,860(19.8s) — 건강한
  ...
  5초 이내, 최대 22.0초.
- **잔여 TIMEOUT 233건은 전수 분류됐다(2026-08-12) — 68%는 예산의 문제가 아니라
  증명 대상이 거짓이다.** 방법·증인은
  [sweeps/witness/WITNESSES.md](sweeps/witness/WITNESSES.md) ⑶⑷:
  - **decode-독 157건**(소형 26 + 대형 131, `sweeps/decode_membership.tsv`):
    폐포에 `$bitstr_to_int`의 **w=0 진동 루프**(`0→−1→0`, TRS 자신의 규칙 56스텝
    시뮬레이션으로 실증 — 잘 정렬된 항이라 과대근사와 무관한 진짜 비종료)를
    품는다. **SN이 거짓이므로 어떤 예산으로도 YES 불가.** w=0 규명(todo.md
    2026-07-09 항목)이 해소되기 전까지 재질의는 낭비다.
  - **빈-패턴 실루프 2건**(`$strip_prefix_rec`/`$strip_suffix_rec`): `prefix=""`
    에서 가드 항진·스트립 항등 → 길이 1 순환(실증). 0-경계 교훈의 텍스트판 —
    스펙 가드(`|prefix| > 0`)감이다.
  - **arith-blind 13건**(≤500규칙): 재귀 측도가 이진 자릿수 산술(`sub(n,1)`,
    상한 향해 `add(n,1)`, `div`) 속에 있어 도구가 못 본다. 실제 종료. 평평한
    300초 재질의도 **13/13 TIMEOUT**(`term_arith13_300.tsv`) — 예산이 아니라
    인코딩의 문제임이 확인됨(닫으려면 측도 lift가 필요).
  - **대형 clean 61건**(>500규칙): 알려진 독 없음 — 예산 확대나 분해가 의미 있는
    유일한 구간.
  - 이 분류가 낡은 인용 하나를 바로잡는다: "`$bin_minus`가 앵커에서 예산 1800으로
    YES"는 **masking 시절 표면**(decode 루프가 가려져 있던)의 판정이라, 큰
    예산의 근거가 아니라 무효화된 열의 유물이다. 현행 표면에서 `$bin_minus`는
    decode-독이라 YES가 불가능하다.
- **`Program_ok`(20,399규칙)의 12시간 런도 이 분류로 재해석된다**: 평평한 예산
  43,200초에서 TIMEOUT 43,210.7초(2026-08-11, `term_program_ok.tsv`)였는데,
  `Program_ok`은 decode-독이라 **YES는 애초에 불가능한 목표였다**. 그 런이 여전히
  증명하는 것: holds_ 분할 전 표면(11,039규칙)의 같은 12시간이 **NO**(holds_
  루프를 AProVE가 실제로 찾음)였던 자리에서, 분할 후 12시간 동안 **어떤 반례도
  다시 나오지 않았다** — 루프 제거의 최대 규모 확인. (decode 루프는 w=0 특정
  인스턴스라 20k규칙 속에서 AProVE의 비종료 탐색이 못 찾는다 — TRS 규칙
  시뮬레이션이 증인을 만든 이유다.)
