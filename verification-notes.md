...
AProVE를 반복 호출하고, 처음 답이 나온 rung에서 멈춰 **그 예산을 기록**한다
(`Termination.budget_ladder`). 예산 단계(rung)는 5에서 4배씩 커지고 **마지막 단계는 항상
`--budget` cap**이다 — 사다리가 바닥나면 종전의 단발 cap 실행과 정확히 같은 실행이 되므로
**판정은 바뀔 수 없다**. 답이 아닌 판정(MAYBE/TIMEOUT/Error)은 전부 다음 단계로 올라간다.
TSV는 5번째 열(`term예산`)이 늘었고, resume은 0열만 읽으므로 옛 4열 파일과도 호환된다.

**두 시간 열의 의미 (2026-07-24 정정, `a6f056ed`).** `term초`는 **답이 나온 예산 단계
하나의 벽시계**이고, 그 아래에서 실패한 단계들은 빠져 있다. 처음 구현은 사다리 전체를
쟀는데 그러면 `$bitacc_offset_op`이 29.2초로 찍혔다 — 실패한 예산 5의 6초 + 증명한 예산
20의 21초다. 승자 단계만 재면 21.6초다. 탐색 비용을 심볼의 비용으로 보고하는 건 이 탐색이 없애려던 바로 그 혼동이라
승자 단계만 재도록 고쳤다(21.4초). AProVE를 아예 안 돌린 행(DEGENERATE, unravel 실패)은
`0.0` 대신 `-`다. 스윕 전체 비용이 필요하면 `term예산`이 지목하는 단계들의 합으로 복원된다.
따라서 **순위·비교에는 `term예산`, 개별 증명 비용에는 `term초`** 를 보면 된다.
...
불변**. 예산은 151심볼이 5, 2심볼(`$bitacc_range_op`·`$bitacc_offset_op`)이 20이다. 답한
단계의 벽시계 합은 **279.8초**로, 종전 열이 재던 예산 300 고정의 합 13,000.9초(3.61시간)와는
잴 대상 자체가 다르다 — 종전 값이 그 심볼의 난이도가 아니라 우리가 준 예산을 재고 있었다는
게 요지다.

**옛 표의 "어려운 심볼" 순위는 전부 예산 아티팩트였다.** 상위 5개의 새 값:

| symbol | 옛 term초(예산 300 고정) | 새 term초(답한 단계) | 새 예산 |
|---|---|---|---|
| `$bin_minus` | 1800.5 | 5.6 | 5 |
| `$write_bits_from_value` | 1320.4 | 6.0 | 5 |
| `$bin_shr` | 420.2 | 5.8 | 5 |
| `$write_value_from_bits_prime` | 321.7 | 5.7 | 5 |
| `$bitacc_offset_op` | 319.7 | 21.6 | 20 |
...
cap 1800): 124/124 YES, 전부 예산 5, 답한 단계의 벽시계 합 479.6초.** 종전 §2가 이고 있던 `term(B)`
...
- [x] **§2 >500 term 열 갱신** (2026-07-24, `a1fd043b`). 27행 stale 표를 124행으로 교체하고
  ...
  전부 예산 5, 합 479.6초**. 모듈러 축의 MAYBE 11건은 전부 닫혔다.
