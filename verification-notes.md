...
**886~2490규칙 108심볼: 예상대로 전부 TIMEOUT (2026-07-27~30, 35시간). 여기서 스윕을 세웠다.**
아래 50심볼 이후 58심볼(1,206~2,490규칙)을 더 돌렸고 **58/58 TIMEOUT**이다 — 대부분
2,145~2,300초(base 예산만), 정규화 재시도까지 간 8건이 4,199~4,633초. 874규칙 위에서 예외가
하나도 없었고, 다음 심볼(`$add_var_i`)부터는 5만 규칙대라 같은 예산에서 같은 결과가 반복될
뿐이므로 **2026-07-30에 스윕을 정지**했다. 재개는 예산을 늘리는 게 아니라 슬라이스를 줄이는
설계(추가 pruning / 모듈러 분해)가 선 뒤에 하는 게 맞다. 아래는 그 앞 50심볼의 기록이다.

**886~1203규칙 50심볼: 예상대로 전부 TIMEOUT (2026-07-27~29, 15.7시간).** 위 예측("남은
심볼은 TIMEOUT 행만 쌓일 공산")을 그대로 확인했다 — 이어서 돈 50심볼이 **50/50 TIMEOUT**이고
시간은 2058~2157초에 몰려 있다(정규화가 슬라이스를 안 바꿔 base 예산 2040초만 쓴 값). 예외 4건
(`$split_externConstructorOrMethodPrototypeList` 4142 · `$name_annotation` 4273 ·
`$name_annotation_default` 4249 · `$typedExpressionIR_as_typedLvalueIR` 4329)은 정규화가 실제로
슬라이스를 바꿔 재시도까지 간 경우로, 상한 4080초 부근이다. **비합류 witness는 하나도 없다** —
전부 예산 소진이다. 남은 346심볼은 더 크므로(중앙값 5만 규칙) 같은 예산으로는 같은 결과가
나온다: 이 축을 더 진행하려면 예산이 아니라 **슬라이스를 줄이는 쪽**(추가 pruning/모듈러 분해)이
필요하다.

**`$write_value*` 5형제 — 스펙 수정 후 재측정 (2026-07-29).** 표의 옛 `YES* (~252초)`는
`n_var = 0` 가드가 없던 스펙에서 잰 값이라 stale이다. 가드를 넣은 뒤 다시 쟀다. 그 과정에서
가드 **철자**가 이 5행을 좌우한다는 걸 확인했다: `-- if n_var =/= 0`은 일반 구조 등식 `eq`의
정의 테이블(50,629규칙)을 의존성 폐포로 끌어와 슬라이스를 **271 → 50,900규칙**으로 키우고,
CRC는 5심볼 전부 5400초 outer kill에서 verdict 없이 끝났다. `-- if $(n_var > 0)`은 산술
프리루드의 `leq`만 쓰므로 슬라이스가 271규칙 그대로다(자세한 기전은
[todo.md](spectec/lib/rewrite/todo.md) 2026-07-29 항목). 표의 값은 `> 0` 판을 심볼당 fresh
프로세스로 잰 것이되, 큰 심볼 스윕이 동시에 돌고 있었으므로 **시간은 상한으로 읽어야 한다**
(판정은 프로세스 독립이라 영향 없음).

**판정은 안 움직였다**: 5행 모두 `YES*` 261.7~278.1초로, 수정 전 251.4~261.2초와 같은 판정에
동시 부하만큼의 차이다. base CRC는 가드가 있어도 여전히 MAYBE(`$write_value_from_bits_prime`
118.8초)이므로, 이 다섯의 MAYBE 원인은 tuple 바인더의 **결정성 임계쌍**이지 `n_var = 0`
오버랩이 아니었다 — 표·CLAUDE.md가 그렇게 적어 뒀던 부분은 이 측정으로 정정한다. 스펙 수정의
가치는 CRC 열이 아니라 **절 순서에 의존하던 실제 결함을 없앤 것**에 있다.
