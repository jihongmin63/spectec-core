...
## 표 읽는 법 (`verification.md` 범례)

각 셀은 `판정 (초s)` — 판정과 그 판정에 걸린 심볼당 **직렬 fresh** 벽시계다.
...
2026-07-24부로 측정 중단·열 삭제**(사유·이력은 위 인용 절). 규칙 수 748 이상 행의 confluence는
...
찍으므로, 마감에 걸린 행의 초는 비용이 아니라 rung 그 자체다. 표에서 그런 행은
`$find_typeDef_i`(1289.6초) 하나뿐이며 그 값은 **상한**이다.

**종합 표**: 축별 판정 분포와 계. `DEGENERATE`(규칙 0)는 termination 열에서만 따로 세고,
confluence 쪽에서는 그 9행이 `-`(미측정) 135에 포함된다. 「합계 초」는 답을 낸 단계의
벽시계 합이라 termination에만 있다 — confluence는 전수 재측정이 진행 중이라 합을 안 낸다.
...
confluence는 `bff805ec` (2026-07-23 전수 재측정) — 통합 커밋 `160fef97`이 분석 표면을
byte-identical로 보존하므로 유효하나, `bdceb303`은 분석 표면의 조건 방향을 바꾸므로
**confluence 열은 뒤집힌 조건을 포함한 슬라이스에 한해 stale**이다(대상·경위는 아래
「confluence 열의 stale 범위」).
...
**confluence 재측정 — 부분 완료 (2026-07-24, 게이트 커밋 `bc65cfb4`).**
`confluence --all --crc-normalize --timeout 1800`을 워크트리 `wll-check`의 바이너리로
돌려 **크기 오름차순 126행**(≤91규칙)까지 측정했다. 결과 TSV는 `/tmp/crc-regate/crc.tsv`
(`--out`이라 resume 가능; MFE/Maude는 워크트리에 없어 `--mfe-dir`/`--maude-bin`으로
메인 체크아웃 것을 가리켜야 한다).

- **126행 전부 기존 표와 일치 — 변화 0.** YES 117 + CRC verdict 없음 9(규칙 0인
  DEGENERATE, 표의 `-`). 합 324.1초. 즉 `orient_conds`의 조건 방향 변경은 이 구간의
  CRC 판정을 바꾸지 않았다.
- **승격 게이트는 아직 개입한 적이 없다.** 126행이 전부 base에서 결론이 나서
  `--crc-normalize` 재시도가 한 번도 안 일어났고, 새 5·6열(WLL verdict / 사용된 정규화)이
  전 행 `-`다. 게이트 효과가 관측되려면 MAYBE/TIMEOUT이 나오는 큰 슬라이스까지 가야 한다.
- **중단 지점은 `$write_bits_from_value`(103규칙)** — 기존 표에서 `TIMEOUT (>1800s)`인
  바로 그 심볼이고, 20분을 넘겨 감시가 프로세스를 끊었다(심볼당 20분 상한). 남은 구간은
  TIMEOUT 12건이 각각 base+normalized로 최대 1시간씩 쓰므로 별건 예산이 필요하다.
